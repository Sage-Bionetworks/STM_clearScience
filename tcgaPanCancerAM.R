...
## LOG INTO SYNAPSE IF NOT SPECIFIED IN .Rprofile
##synapseLogin()

if(!require(cafr)){
  require(devtools)
  install_github(repo="cafr", username="weiyi-bitw")
	require(cafr)
}
if(!require(impute)){
...
panCancerAttractorSearch <- function(synList, seed, a=5, verbose=TRUE){
	...
	#===== Impute missing values =====
		...
		ge <- as.matrix(read.delim(file.path(syn$cacheDir, syn$files), row.names=1, check.names=F ))
		ge[ge=="null"] <- NA
	...
	#===== find attractor and store only top 50 genes =====
	...
	scores <- sort(scores, decreasing=T)[1:50]
...
## THESE ARE THE LIST OF SYNAPSE IDS THAT ARE SEARCHED OVER
